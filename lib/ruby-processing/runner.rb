...
module Processing
  ...
  Examples:
    ...
    rp5 watch some_new_sketch.rb [path/to/include [another/path]]
  ...
  Everything Else:
    ...
    def spin_up(starter_script, sketch)
      ...
      command = "java #{java_args} -cp \"#{jruby_complete}\" #{dock_icon} org.jruby.Main \"#{runner}\" #{sketch} #{@options.args.join ' '}"
      puts command
      exec(command)
